...
function balancedNum(number) {
  let arr = number.toString().slice('');
  return arr.slice(0, Math.round(arr.length / 2) - 1).split('').reduce((a, b) => +a + +b, 0) === arr.slice(arr.length / 2 + 1).split('').reduce((a, b) => +a + +b, 0) ? 'Balanced' : 'Not Balanced';
}

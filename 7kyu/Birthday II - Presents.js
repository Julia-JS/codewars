...
function present(x, y){
  switch (x) {
    case 'goodpresent': return [...x].map(a => String.fromCharCode(a.charCodeAt() + y)).join('');
    case 'crap': case 'empty': return [...x].sort((a, b) => a.charCodeAt() - b.charCodeAt()).join('');
    case 'bang': return [...x].map(a => a.charCodeAt() - y).reduce((a, b) => a + b);

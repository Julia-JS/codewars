...
function noonerize(numbers) {
  if (numbers.filter(a => typeof a !== 'number').length) {
    return 'invalid array';
  }

  const firstNumber = +(numbers[1].toString()[0] + numbers[0].toString().slice(1));
  const secondNumber = +(numbers[0].toString()[0] + numbers[1].toString().slice(1));

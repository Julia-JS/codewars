...
const getTurkishNumber = (num) => {
  ...
  let dozens = ['on', 'yirmi', 'otuz', 'kırk', 'elli', 'altmış', 'yetmiş', 'seksen', 'doksan'];
  return num ? `${Math.trunc(num / 10) ? dozens[Math.trunc(num / 10 - 1)] : ''}${Math.trunc(num / 10) && num % 10 ? ' ' : ''}${num % 10 ? units[num % 10] : ''}` : units[0];
}

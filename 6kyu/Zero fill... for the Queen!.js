...
// There are many ways to achieve this. Additionally, if the size passed is less than the length of number, just return the number as a string.
// Let's have one that would be both useful and good performance-wise.
//
// NOTES:
...
function zeroFill(number, size) {
  return (Math.floor(Math.abs(number)).toString()).padStart(size, 0);
}

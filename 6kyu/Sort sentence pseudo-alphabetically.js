...
function sort(sentence){
  const arr = sentence.replace(/[^a-z ]/ig, '').split(' ');
  const lowers = arr.filter(a => a[0] === a[0].toLowerCase()).sort();
  const uppers = arr.filter(a => a[0] === a[0].toUpperCase()).sort().reverse();
  return lowers.concat(uppers).join(' ');
}

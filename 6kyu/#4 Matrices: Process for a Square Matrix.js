...
function avgDiags(m) {
  ...
  let avg1 = Math.round(d1.filter((a, i) => i % 2 && a >= 0).reduce((a, b) => a + b, 0) / d1.filter((a, i) => i % 2  && a >= 0).length);

  let d2 = [];
  for (let i = 0; i < m.length; i++) {
    d2.push(m[m.length - i - 1][i]);
  }
  let avg2 = Math.round(Math.abs(d2.filter((a, i) => !(i % 2) && a < 0).reduce((a, b) => a + b, 0) / d2.filter((a, i) => !(i % 2) && a < 0).length));


  return [Number.isNaN(avg1) ? - 1 : avg1, Number.isNaN(avg2) ? - 1 : avg2];
}

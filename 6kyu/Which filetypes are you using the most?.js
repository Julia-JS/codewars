...
function solve(files) {
  let arr = files.map(a => '.' + a.split('.')[a.split('.').length - 1]);
  let obj = arr.reduce((a, i) => {a[i] = a[i] ? a[i] + 1 : 1; return a}, {});
  let keys = Object.keys(obj);
  let array = keys.sort((a, b) => obj[b] - obj[a]);
  let maxN = obj[array[0]];
  return Object.keys(obj).filter(key => obj[key] === maxN).sort();
}

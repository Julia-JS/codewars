...
function dayOfWeek(birthdayDate) {
  const [month, day, year] = birthdayDate.split('-');
  ...
  for (let i = 1; ; i++) {
    ...
    const nextDay = nextBirthdayDate.getDate();
    nextDayOfWeek = nextBirthdayDate.getDay();
    if (dayOfWeek === nextDayOfWeek && +month === nextMonth + 1 && +day === nextDay) {
      return i;
    }
  }
}

...
function palindromization(element, n){
  ...
  let h1 = element.repeat(Math.ceil(n / (2 * element.length))).slice(0, Math.ceil(n / 2));
  let h2 = element.repeat(Math.ceil(n / (2 * element.length))).slice(0, Math.floor(n / 2)).split('').reverse().join('');
  return h1 + h2;
}
